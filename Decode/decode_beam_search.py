...
def beam_decode_sentence(hidden_X, config, num_generate=1, beam_size=5, batch_size=1):
    ...
    :param hidden_X: hidden_X of sentence embedding (N, embed_dim) with/without projection
    ...
    EOS_token = eos
    hidden_X_unsqueeze = torch.unsqueeze(hidden_X, 0)  # [1, N, embed_dim]
    ...
    for idx in range(batch_size):
        ...
        node = BeamSearchNode(
            past, None, torch.tensor([[220]]).cuda(), 0, 1
        )  # 220 refers to single space ' ' on GPT
        nodes = PriorityQueue()
        ...
        for text_len in range(50):
            ...
            if text_len == 0:
                logits, past = model(
                    inputs_embeds=hidden_X_unsqueeze,
...
def greedy_decode(decoder_hidden, encoder_outputs, target_tensor):
    ...
    decoder_input = torch.LongTensor(
        [[SOS_token] for _ in range(batch_size)], device=device
    )
