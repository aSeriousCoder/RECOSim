...
class RandomRecSys(BaseRecSys):
    ...
    def recommend_post(self):
        self._logger.debug("Randomly Recommending News ...")
        # window中采样
        ...
        candidate_mat = torch.stack([id_post_up_to_now[candidate_list.long()] for candidate_list in candidate_mat], dim=0)
        candidate_mat = torch.sort(candidate_mat, descending=False, dim=1).values  # 对每个user的召回，按照id排序
        ...
        random_ranking_score = torch.rand_like(filter_out_mat.float()).abs()
        selected = torch.topk((1-filter_out_mat.float())*random_ranking_score, dim=1, k=self._post_rec_list_length).indices
        random_rec_list = candidate_mat.gather(1, selected)
        random_rec_list_in_pair = torch.stack([
            torch.tensor(list(range(self._graph.graph.num_nodes('user')))).unsqueeze(1).expand(-1, self._post_rec_list_length).reshape(-1), 
            random_rec_list.reshape(-1)
        ], dim=1)
        return random_rec_list_in_pair

    def recommend_friends(self):
        self._logger.debug("Randomly Recommend Friends ...")
        candidate_mat = torch.randint(0, self._graph.graph.num_nodes('user'), [self._graph.graph.num_nodes('user'), self._friend_rec_list_length * 10])
        candidate_mat = torch.sort(candidate_mat, descending=False, dim=1).values  # 对每个user的召回，按照id排序
        ...
        random_ranking_score = torch.rand_like(filter_out_mat.float()).abs()
        selected = torch.topk((1-filter_out_mat.float())*random_ranking_score, dim=1, k=self._friend_rec_list_length).indices
        random_rec_list = candidate_mat.gather(1, selected)
        random_rec_list_in_pair = torch.stack([
